...
/**
 * Provides product list state, client-side filtering and sorting, loading, and order persistence for a specific list.
 *
 * @param listId - Identifier of the list whose products should be loaded and managed
 * @param sortOrder - Sort order to apply when producing the filtered product list
 * @param searchQuery - Text used to filter products; matched via preprocessing and adaptive similarity thresholding
 * @returns An object with:
 *  - `products`: the raw product state array (kept for usages such as list extraData)
 *  - `filteredProducts`: the current filtered and sorted product array computed from `products`, `searchQuery`, and `sortOrder`
 *  - `setProducts`: state setter for `products`
 *  - `loadProducts`: function to reload products from the data source
 *  - `handleProductOrderChange`: function that accepts a new product order, applies it optimistically, and persists the order
 *  - `saveProductHistory`: utility exported for saving product history
 */
